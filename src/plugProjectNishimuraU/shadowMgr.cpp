...
{
	OSReport("ShadowMgr::update()\n");
	if (mEnabled) {
		for (int i = 0; i < getViewportCount(); i++) {
			getActiveCylinder()->setCameraParms(getViewport(i)->mCamera, i);
		}
			...
			if (node->getGameObject()->needShadow()) {
				...
				for (int i = 0; i < getViewportCount(); i++) {
					if (getViewport(i)->viewable()) {
						...
						Matrixf* viewMat = getViewport(i)->mCamera->getViewMatrix(false);
