...
{
	if (sys->mPlayData) {
		mIsRumbleActive = (sys->mPlayData->mSoundMode != 0);
	} else {
		mIsRumbleActive = true;
	}
...
bool RumbleMgr::isStartAndEnd(int* startEndArray, int rumbleID)
{
	if (rumbleID == RUMBLEID_ALL) {
		...
		startEndArray[1] = 2;
		return true;
	} else if (rumbleID == RUMBLEID_Navi2) {
		startEndArray[0] = 2;
		startEndArray[1] = 3;
		return true;
	} else if (rumbleID == RUMBLEID_Navi3) {
		startEndArray[0] = 3;
		startEndArray[1] = 4;
		return true;
	}
