...
{
	mCurrPlayerMode     = -1;
	mCameraVibrateState = new int[4];
	mViewportList       = new Viewport*[4];
	mCameraObjList      = new PlayCamera*[4];
...
bool CameraMgr::isStartAndEnd(int* naviIDs, int type)
{
	...
	if (type == CAMNAVI_Louie) { // just louie/president
		...
		naviIDs[1] = NAVIID_President;
		return true;
	}

	if (type == CAMNAVI_President) {
		naviIDs[0] = NAVIID_President;
		naviIDs[1] = NAVIID_Wife;
		return true;
	}

	if (type == CAMNAVI_Wife) {
		naviIDs[0] = NAVIID_Wife;
		naviIDs[1] = NAVIID_Multiplayer;
		return true;
	}
