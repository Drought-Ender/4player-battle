...
void ShowOption(OptionsEnum option) { gOptions[option].hide = false; }
...
void Option::ReadOptions(Stream& stream)
{
	...
	for (int i = 0; i < cardCount; i++) {
		...
		if (Game::VsGame::VsSlotCardMgr::sAllCards[i]) {
			for (int j = 0; j < varForward; j++)
				Game::VsGame::VsSlotCardMgr::sAllCards[i]->varibleForward();
		}
	}
...
int GetConfigSize() { return ARRAY_SIZE(gConfig); }
...
bool VsConfigMenu::update(VsOptionsMenuMgr* menu)
{
	...
	if ((menu->mController->isButtonDown(JUTGamePad::PRESS_A) && mCursorOptionIndex == endIdx)
	    || menu->mController->isButtonDown(JUTGamePad::PRESS_START | JUTGamePad::PRESS_B)) {
...
bool VsOptionsMenuMgr::checkToggleTournament()
{
	...
	// OSReport("Info: %i, %i, %i, %i\n", mController->isButtonDown(tournamentButtonChord), mController->getButton(),
	// mController->getButton() & tournamentButtonChord, (mController->getButton() & tournamentButtonChord) == tournamentButtonChord);
...
void CharacterImage::read(Stream& stream) { mCharacterName = stream.readString(nullptr, 0); }
...
char* CharacterImage::getDisplayName() { return mCharacterName; }
...
void CharacterSelect::draw(VsOptionsMenuMgr* menu, Graphics& gfx)
{
	...
	static JUtility::TColor colors[4] = { 0xff0000ff, 0x0000ffff, 0xffff00ff, 0x00ff00ff };
...
void GamemodeSelect::cleanup() { gGameModeID = (EMainGamemodes)mActiveMainGamemodeID; }
