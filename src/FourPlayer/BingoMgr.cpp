...
BingoMgr::BingoMgr() { mCards = new BingoCard[4]; }

BingoMgr::~BingoMgr() { delete[] mCards; }
...
void BingoMgr::read(Stream& stream) { mKey.read(stream); }
...
DroughtLib::StaticSizedArray<16, f32> BingoMgr::BingoCard::CalcPlacementWeights(ObjectKey& key, LineWeightReport& weightReport,
                                                                                bool evalSelf)
{
...
void BingoMgr::BingoCard::ReweighCardByLineShuffle(ObjectKey& key)
{
	...
	while (FABS(strongestLineWeight - expectedLineWeight) > 0.4f && attempts < 100) {
		...
		DroughtLib::StaticSizedArray<ArrSize, int> indexList
		    = DroughtLib::ArrayFunctions::MakeSortedIndexList<ArrSize>(weightReport.mWeight);
...
int BingoMgr::ObjectKey::FindPellet(Pellet* pellet) { return FindPellet(pellet->getKind(), pellet->mConfig->mParams.mIndex); }
...
int BingoMgr::ObjectKey::FindEnemy(EnemyBase* enemy) { return FindEnemy(enemy->getEnemyTypeID()); }
...
void BingoMgr::CountExists() { mKey.CountExists(); }
...
void BingoMgr::ObjectKey::CountExists()
{
	...
	FOREACH_NODE(EnemyMgrNode, generalEnemyMgr->mEnemyMgrNode.mChild, enemyNode)
	{
		...
		for (int i = 0; i < enemyNode->mMgr->mNumObjects; i++) {
			...
			if (enemy && !enemy->mPelletDropCode.isNull()) {
				...
				// const char* peltName =
				// PelletList::Mgr::getConfigList((PelletList::cKind)pelType)->getPelletConfig(dropCode)->mParams.mName.mData; const char*
				// enemyName = EnemyInfoFunc::getEnemyName(enemy->getEnemyTypeID(), 0xffff); DebugReport("Found %s in %s\n", peltName,
				// enemyName);
	...
	//     const char* peltName =
	//     PelletList::Mgr::getConfigList((PelletList::cKind)mObjectEntries[idx].mPelType)->getPelletConfig(mObjectEntries[idx].mPelletID)->mParams.mName.mData;
...
void BingoMgr::exec(VsGameSection* section)
{
	...
	if (gameSystem->paused() || section->mTimeLimit <= 0.0f || section->mMenuFlags || !gameSystem->isFlag(GAMESYS_Unk6)
	    || gameSystem->paused_soft() || moviePlayer->mDemoState) {
		return;
	}
