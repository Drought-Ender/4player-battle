...
void BuffMgr::addBuff(NaviBuffBase* buff, u8 naviID) { mBuffData[naviID].mContainer.add(buff); }
...
PluckSpeedBuff::PluckSpeedBuff(u8 v)
    : mType(v)
{
...
{
	FOREACH_NODE(NaviBuffBase, mContainer.mChild, node) { node->update(); }
}
...
void PluckSpeedBuff::update() { mAnimationTimer += getSpeedBuff() * sys->mDeltaTime * 0.5f; }
