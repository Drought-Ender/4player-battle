...
float Pellet::buryBedamaVs()
{
	...
	if (mPelletFlag == FLAG_VS_BEDAMA_RED || mPelletFlag == FLAG_VS_BEDAMA_BLUE || mPelletFlag == FLAG_VS_BEDAMA_WHITE
...
// canAutopluck__Fv
bool canAutopluck() { return gConfig[AUTOPLUCK] == ConfigEnums::AUTOPLUCK_ON || gConfig[AUTOPLUCK] == ConfigEnums::AUTOPLUCK_CANCEL; }
...
void NaviWalkState::collisionCallback(Navi* navi, CollEvent& event)
{
	...
	if (moviePlayer->mDemoState == 0 && gameSystem->isVersusMode() && canPickup(navi, collider) && !collider->mCaptureMatrix
	    && collider->isAlive() && navi->mController1) {
