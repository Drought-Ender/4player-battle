...
float interactive(PSM::ActorDirector_Scaled* director)
{
    ...
    if (actor != 0)
    {
        ...
        p1dist = (actorPos.x - p1Pos.x) * (actorPos.x - p1Pos.x) + (actorPos.y - p1Pos.y) * (actorPos.y - p1Pos.y) + (actorPos.z - p1Pos.z) * (actorPos.z - p1Pos.z);
        p2dist = (actorPos.x - p2Pos.x) * (actorPos.x - p2Pos.x) + (actorPos.y - p2Pos.y) * (actorPos.y - p2Pos.y) + (actorPos.z - p2Pos.z) * (actorPos.z - p2Pos.z);
        p3dist = (actorPos.x - p3Pos.x) * (actorPos.x - p3Pos.x) + (actorPos.y - p3Pos.y) * (actorPos.y - p3Pos.y) + (actorPos.z - p3Pos.z) * (actorPos.z - p3Pos.z);
        p4dist = (actorPos.x - p4Pos.x) * (actorPos.x - p4Pos.x) + (actorPos.y - p4Pos.y) * (actorPos.y - p4Pos.y) + (actorPos.z - p4Pos.z) * (actorPos.z - p4Pos.z);
