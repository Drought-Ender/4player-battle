...
{
	shadowParam.mPosition = mKoshiJoint->getWorldMatrix()->getTranslation();
...
bool Obj::turnFunc()
{
	f32 angle = turnToTarget2_tamago(mGoalPosition, mRandomTurnFactor * *C_GENERALPARMS.mTurnSpeed(),
	                          mRandomTurnFactor * *C_GENERALPARMS.mMaxTurnAngle());
	if (FABS(angle) < 0.1f) {
		return true;
	}
...
void Obj::ballMove()
{
	f32 vel = FABS((mCurrentVelocity.y * 2.0f) / 50.0f);
	if (vel > 2.0f) {
		vel = 2.0f;
	}
