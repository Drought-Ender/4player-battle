 ...
 * @note Size: 0x34
 */
LifeGauge::LifeGauge() { init(MAX_LIFEGAUGE_SEGMENTS, MAX_LIFEGAUGE_SEGMENTS_BLUE); }
...
void LifeGauge::draw(f32 radius, f32 centerX, f32 centerY)
{
	...
	for (int i = 0; i < mCurrentSegmentNum; i++) {
		...
		f32 angle     = -HALF_PI - (TAU * ((f32)i / static_cast<f32>(mMaxSegmentNum)));
		vertices[1].x = (greenRadius * (f32)cos(angle)) + vertices[0].x;
		vertices[1].y = (greenRadius * (f32)sin(angle)) + vertices[0].y;
		vertices[1].z = vertices[0].z;

		// third vertex is on outside of circle at "end" of segment
		angle         = -HALF_PI - (TAU * ((f32)(i + 1) / static_cast<f32>(mMaxSegmentNum)));
		vertices[2].x = (greenRadius * (f32)cos(angle)) + vertices[0].x;
		vertices[2].y = (greenRadius * (f32)sin(angle)) + vertices[0].y;
...
void LifeGauge::drawOneQuad(Vector3f* vertices, Color4& color)
{
...
void LifeGaugeMgr::activeLifeGauge(Game::Creature* obj, f32 healthRatio)
{
	...
	if (list) {
		...
		list->mLifeGauge.mCurrentTimerSegmentNum = 0;
...
void LifeGaugeMgr::inactiveLifeGauge(Game::Creature* obj)
{
	...
	if (list) {
		...
		list->mLifeGauge.mMaxSegmentNumTimer     = MAX_LIFEGAUGE_SEGMENTS_BLUE;
		list->mLifeGauge.mCurrentTimerSegmentNum = 0;
...
void LifeGaugeMgr::update()
{
	for (LifeGaugeList* list = mListActive.mNext; list; list = list->mNext) {
		...
		if (list->mParam.mIsGaugeShown) {
			...
			list->mLifeGauge.update(list->mParam.mCurrHealthRatio, list->mParam.mCurrTimerRatio);
		}
		if (list->mParam.mCurrHealthRatio == 1.0f && list->mParam.mCurrTimerRatio == 0.0f) {
...
void LifeGaugeMgr::draw(Graphics& gfx)
{
	...
	if (!Game::moviePlayer || !Game::moviePlayer->isFlag(Game::MoviePlayer::IS_ACTIVE)) {
		LifeGaugeList* list;
		if (mListActive.mNext) {
