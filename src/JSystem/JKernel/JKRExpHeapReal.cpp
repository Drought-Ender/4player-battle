 ...
 * @note Size: 0x80
 */
JKRExpHeapReal* JKRExpHeapReal::createRoot(int maxHeaps, bool errorFlag)
{
	JKRExpHeapReal* heap = nullptr;
	if (!sRootHeap) {
		void* memory;
		u32 memorySize;
		initArena((char**)&memory, &memorySize, maxHeaps);
		u8* start       = (u8*)memory + ALIGN_NEXT(sizeof(JKRExpHeapReal), 0x10);
		u32 alignedSize = memorySize - ALIGN_NEXT(sizeof(JKRExpHeapReal), 0x10);
		heap            = new (memory) JKRExpHeapReal(start, alignedSize, nullptr, errorFlag);
		sRootHeap       = heap;
	}
	heap->_6E = true;
	return heap;
 ...
 * @note Size: 0xE4
 */
JKRExpHeapReal* JKRExpHeapReal::create(u32 size, JKRHeap* parent, bool errorFlag)
{
	JKRExpHeapReal* newHeap;
	if (!parent) {
		parent = sRootHeap;
	}
	...
	u32 expHeapSize = ALIGN_NEXT(sizeof(JKRExpHeapReal), 0x10);
	if (alignedSize < 0xa0)
		return nullptr;
	...
	newHeap = new (memory) JKRExpHeapReal(dataPtr, alignedSize - expHeapSize, parent, errorFlag);
	if (newHeap == nullptr) {
		JKRFree(memory);
		return nullptr;
	};
 ...
 * @note Size: 0x84
 */
void JKRExpHeapReal::do_destroy()
{
	if (!_6E) {
 ...
 * __ct
 */
JKRExpHeapReal::JKRExpHeapReal(void* p1, u32 p2, JKRHeap* p3, bool p4)
    : JKRHeap(p1, p2, p3, p4)
{
	mCurrentAllocMode = 0;
 ...
 * @note Size: 0x68
 */
JKRExpHeapReal::~JKRExpHeapReal() { dispose(); }
 ...
 */
void* JKRExpHeapReal::do_alloc(u32 byteCount, int padding)
{
	OSLockMutex(&mMutex);
	if (byteCount < 4) {
		byteCount = 4;
...
static JKRExpHeapReal::CMemBlock* DBfoundBlock;
static JKRExpHeapReal::CMemBlock* DBnewFreeBlock;
static JKRExpHeapReal::CMemBlock* DBnewUsedBlock;

/**
 * @note Address: 0x8002024C
 * @note Size: 0x254
 */
void* JKRExpHeapReal::allocFromHead(u32 size, int align)
{
 ...
 * @note Size: 0xE4
 */
void* JKRExpHeapReal::allocFromHead(u32 size)
{
	size                  = ALIGN_NEXT(size, 4);
	int foundSize         = -1;
 ...
 * @note Size: 0x158
 */
void* JKRExpHeapReal::allocFromTail(u32 size, int align)
{
	u32 offset            = 0;
	CMemBlock* foundBlock = nullptr;
 ...
 * @note Size: 0xE0
 */
void* JKRExpHeapReal::allocFromTail(u32 size)
{
 ...
 * @note Size: 0x74
 */
void JKRExpHeapReal::do_free(void* p1)
{
	OSLockMutex(&mMutex);
 ...
 * @note Size: 0x94
 */
int JKRExpHeapReal::freeGroup(u8 groupID)
{
	lock();
 ...
 * @note Size: 0x7C
 */
void JKRExpHeapReal::do_freeAll()
{
	lock();
 ...
 * @note Size: 0x90
 */
void JKRExpHeapReal::do_freeTail()
{
	lock();
 ...
 * @note Size: 0x4
 */
void JKRExpHeapReal::do_fillFreeArea() { }

/**
 * @note Address: 0x800209D4
 * @note Size: 0x58
 */
u8 JKRExpHeapReal::do_changeGroupID(u8 groupID)
{
	lock();
 ...
 * Regswaps
 */
int JKRExpHeapReal::do_resize(void* ptr, u32 size)
{
	lock();
 ...
 * @note Size: 0x88
 */
int JKRExpHeapReal::do_getSize(void* p1)
{
	OSLockMutex(&mMutex);
	CMemBlock* block = CMemBlock::getHeapBlock(p1);
 ...
 * @note Size: 0x6C
 */
u32 JKRExpHeapReal::do_getFreeSize()
{
	lock();
	int maxFreeSize = 0;
 ...
 * @note Size: 0x74
 */
void* JKRExpHeapReal::do_getMaxFreeBlock()
{
	OSLockMutex(&mMutex);
	int maxFreeSize         = 0;
 ...
 * @note Size: 0x64
 */
u32 JKRExpHeapReal::do_getTotalFreeSize()
{
	int freeSize = 0;
	OSLockMutex(&mMutex);
	for (CMemBlock* block = mHead; block != nullptr; block = block->mNext) {
...
s32 JKRExpHeapReal::getUsedSize(u8 groupId) const
{
	JKRExpHeapReal* this2 = const_cast<JKRExpHeapReal*>(this);
	this2->lock();
	u32 size = 0;
 ...
 * @note Size: 0x98
 */
void JKRExpHeapReal::appendUsedList(JKRExpHeapReal::CMemBlock* blockToAppend)
{
	if (!blockToAppend) {
 ...
 * @note Size: 0x4C
 */
void JKRExpHeapReal::setFreeBlock(JKRExpHeapReal::CMemBlock* p1, JKRExpHeapReal::CMemBlock* p2, JKRExpHeapReal::CMemBlock* p3)
{
	if (p2 == nullptr) {
		mHead     = p1;
		p1->mPrev = nullptr;
 ...
 * @note Size: 0x34
 */
void JKRExpHeapReal::removeFreeBlock(JKRExpHeapReal::CMemBlock* blockToRemove)
{
 ...
 * @note Size: 0x34
 */
void JKRExpHeapReal::removeUsedBlock(JKRExpHeapReal::CMemBlock* blockToRemove)
{
	// UNUSED FUNCTION
	CMemBlock* prev = blockToRemove->mPrev;
	CMemBlock* next = blockToRemove->mNext;
 ...
 * @note Size: 0x1E4
 */
void JKRExpHeapReal::recycleFreeBlock(JKRExpHeapReal::CMemBlock* block)
{
	JKRExpHeapReal::CMemBlock* newBlock = block;
 ...
 * @note Size: 0x104
 */
void JKRExpHeapReal::joinTwoBlocks(JKRExpHeapReal::CMemBlock* block)
{
...
void genData()
{
	JUTWarningConsole_f("+---------------JKRExpHeapReal\n");
 ...
 * @note Size: 0x20
 */
void JKRExpHeapReal::CMemBlock::initiate(JKRExpHeapReal::CMemBlock* prev, JKRExpHeapReal::CMemBlock* next, u32 size, u8 groupID, u8 alignment)
{
	mUsageHeader    = 'HM';
	mFlags          = alignment;
	mGroupID        = groupID;
 ...
 * @note Size: 0x44
 */
JKRExpHeapReal::CMemBlock* JKRExpHeapReal::CMemBlock::allocFore(u32 size, u8 groupId1, u8 alignment1, u8 groupId2, u8 alignment2)
{
 ...
 * @note Size: 0x58
 */
JKRExpHeapReal::CMemBlock* JKRExpHeapReal::CMemBlock::allocBack(u32 size, u8 groupID, u8 p3, u8 allocGroupID, u8 p5)
{
	CMemBlock* newBlock = nullptr;
	if (mAllocatedSpace >= size + sizeof(CMemBlock)) {
 ...
 * @note Size: 0x60
 * free__Q210JKRExpHeapReal9CMemBlockFP10JKRExpHeapReal
 */
void* JKRExpHeapReal::CMemBlock::free(JKRExpHeapReal* heap)
{
 ...
 * @note Size: 0x1C
 */
JKRExpHeapReal::CMemBlock* JKRExpHeapReal::CMemBlock::getHeapBlock(void* mem)
{
 ...
 * @note Size: 0xF4
 */
void JKRExpHeapReal::state_register(JKRHeap::TState* p, u32 param_1) const
{
	...
	if (param_1 <= 0xff) {
		...
		setState_uUsedSize_(p, getUsedSize_((JKRExpHeapReal*)this));
	}
	u32 checkCode = 0;
	for (CMemBlock* block = mHeadUsedList; block; block = block->mNext) {
		if (param_1 <= 0xff) {
 ...
 * @note Size: 0x30
 */
bool JKRExpHeapReal::state_compare(const JKRHeap::TState& r1, const JKRHeap::TState& r2) const
{
	bool result = true;
